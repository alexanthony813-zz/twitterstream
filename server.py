...
def connect():
    ...
    if not is_prod:
        global connection
        # os.fork()
        connection = MongoClient(uri, port=23442, maxPoolSize=1, waitQueueMultiple=10, connect=False)
    else:
        connection = MongoClient('localhost', MONGO_DEV_PORT, maxPoolSize=1, waitQueueMultiple=10, connect=False)
